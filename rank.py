#!/usr/bin/env python3
"""RedRob India Runs Track 1 candidate ranker.

The ranking step is deliberately CPU-only, deterministic, and offline. It reads
JSONL records, scores candidates against the released Senior AI Engineer JD, and
writes the required top-100 CSV. No hosted model, network call, or GPU is used.
"""
...
# Terms are derived from the released job description and the dataset schema.
ROLE_TERMS = [
...
LLM_ML_TERMS = [
    ...
    "pytorch", "tensorflow", "huggingface", "fine tuning", "fine tuning",
...
def is_disqualified(candidate: dict[str, Any]) -> bool:
    """Reject obvious title traps; technical evidence cannot rescue them."""
...
def experience_score(years: float) -> float:
    """Score the released JD's flexible 5–9 year band, peaking at 6–8."""
    if years <= 0:
        return 0.0
    if 6.0 <= years <= 8.0:
        return 5.0
...
def score_candidate(candidate: dict[str, Any], as_of: dt.date) -> tuple[float, dict[str, Any]]:
    ...
    retrieval_full = matching_terms(full_text, RETRIEVAL_TERMS)
    ...
    skill_context_score = min(len(retrieval_full) + len(ml_career), 10) * 0.25
    ...
    skills_names = [str(skill.get("name", "")).strip() for skill in candidate.get("skills", []) or [] if str(skill.get("name", "")).strip()]
    matched_skill_names = [name for name in skills_names if any(has_term(normalize_text(name), term) for term in relevant_skill_names)][:4]
    concerns = []
    ...
    features = {
        "raw_score": round(raw_score, 8), "fit_score": round(fit_score, 4), "experience_score": round(exp_score, 4),
        "behavior_modifier": round(behavior_modifier, 4), "behavior_activity": round(behavior["activity"], 4),
        "role_hits": role_hits, "retrieval_hits": retrieval_career, "ml_hits": ml_career,
        "production_hits": production_hits, "evaluation_hits": evaluation_hits, "product_hits": product_hits,
        "evidence": evidence, "matched_skills": matched_skill_names, "concerns": concerns,
        "title": str(profile.get("current_title", "")).strip() or "Unknown title",
        "years": years, "notice_days": int(safe_float((candidate.get("redrob_signals", {}) or {}).get("notice_period_days"), 180)),
        "response": behavior["response"], "inactive_days": int(behavior["inactive_days"]),
        "product_score": product_score, "suspicious_skills": suspicious_skills,
    }
    return round(raw_score, 8), features


def normalise_scores(entries: list[tuple[float, str, dict[str, Any]]]) -> list[tuple[float, str, dict[str, Any]]]:
    if not entries:
        return entries
    scores = [entry[0] for entry in entries]
    low, high = min(scores), max(scores)
    span = high - low or 1.0
    return [(round((score - low) / span, 6), cid, candidate) for score, cid, candidate in entries]
...
def main() -> None:
    ...
    normalized = normalise_scores([(score, cid, candidate) for score, cid, candidate, _ in ranked])
    feature_by_id = {cid: features for _, cid, _, features in ranked}
    args.out.parent.mkdir(parents=True, exist_ok=True)
    with args.out.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.writer(handle, lineterminator="\n")
        writer.writerow(["candidate_id", "rank", "score", "reasoning"])
        for rank, (score, cid, candidate) in enumerate(normalized, start=1):
            writer.writerow([cid, rank, f"{score:.6f}", build_reasoning(candidate, feature_by_id[cid], rank, score)])
    if args.diagnostics:
        diagnostics = {
            "as_of_date": as_of.isoformat(), "scanned": scanned, "skipped": skipped,
            "disqualified": disqualified, "heap_size": len(heap), "output_rows": len(normalized),
            "top100": [
                {"candidate_id": cid, "rank": rank, "raw_score": round(raw, 8), "features": features}
                for rank, (raw, cid, _, features) in enumerate(ranked, start=1)
            ],
        }
        args.diagnostics.write_text(json.dumps(diagnostics, indent=2), encoding="utf-8")
    print(f"Scanned: {scanned:,}; skipped: {skipped:,}; disqualified: {disqualified:,}; wrote: {len(normalized)} rows; as_of: {as_of.isoformat()}")
